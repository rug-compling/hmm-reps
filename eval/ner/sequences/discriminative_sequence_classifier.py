...
class DiscriminativeSequenceClassifier(sc.SequenceClassifier):
    ...
    def compute_scores(self, sequence):
        """
        Prepare trellis for the sequence.
        ...
        #
        #transition_scores = np.zeros([length-1, num_states, num_states, num_states])
        #
        ...
        for tag_id in range(num_states):
             ...
             #initial_scores[tag_id] = self.parameters[initial_features].sum()
        ...
        for pos in range(length):
            for tag_id in range(num_states):
                ...
                #emission_scores[pos, tag_id] = self.parameters[emission_features].sum()
                # capable of real-valued feats
                emission_scores[pos, tag_id] = (self.parameters[list(emission_features.keys())] * np.array(list(emission_features.values()))).sum()
            ...
            if pos > 0:
                for tag_id in range(num_states):
                    for prev_tag_id in range(num_states):
                        ...
                        #transition_scores[pos-1, tag_id, prev_tag_id] = self.parameters[transition_features].sum()
        # Final position.
        for prev_tag_id in range(num_states):
             ...
             #final_scores[prev_tag_id] = self.parameters[final_features].sum()
